...
/**
 * Facade f�r den Datenbankzugriff zur Season via JPA. Implementierung als EJB.
...
public class SeasonFacade extends AbstractFacade<Season> {
	...
	/**
	 * Leerer Konstruktor. Standard f�r den Zugriff via EJB. Die Entit�tsklasse
	...
	/**
	 * Konstruktor f�r JUnit Tests. Die Entit�tsklasse wird der Superklasse
	...
	public Season getSeasonFromDate(final Date datum) {
		...
		try {
			...
			logger.warn("Keine Saison f�r Datum gefunden");
		} catch (NonUniqueResultException nue) {
			logger.error("Mehr als eine Saison f�r Datum gefunden", nue);
		}
	...
	/**
	 * L�scht eine Saison, wenn sie vorhanden ist und keine Referenzen mehr zu
	 * Spielen hat.
	 * 
	 * @param season
	 *            zu l�schende Saison
	...
	public Statusliste deleteSeason(final Season season) {
		...
		if (tmp == null) {
			...
			logger.info("Saison hat noch g�ltige Spiele");
			statusliste.addStatus(MessageConstants.SEASON_HAS_REFERENCES);
		} else {
			logger.info("L�sche Saison");
			remove(season);
	...
	/**
	 * L�scht alle Seasons bevor einem bestimmten Datum.
	 * 
	 * @param date
	 *            Datum, vor dem die Seans gel�scht werden sollen.
